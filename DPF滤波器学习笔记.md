...
因为两个概率密度之间没有共轭关系，我们无法写出一个漂亮的解析解；同时，即使能写出来，结果分布也可能需要无限多的参数才能精确描述，这在工程上是不可行的。本文重点在**校正**；预测仍用常规 GMM 传播（线性化、求积或 Monte Carlo）。地月算例中均值与协方差平方根沿动力学 ODE 与 STM 传播（见论文第 10–11 页）。
