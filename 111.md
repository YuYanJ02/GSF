...
>
> **阅读说明**：LaTeX 已针对 **GitHub 数学渲染**优化：display 公式一律单行、禁用 `\operatorname` / `\mathbb{}`、先验/后验上标用 `\text{-}` / `\text{+}`。
...
| 符号 | 含义 |
|------|------|
| $x_k \in \text{R}^n$ | 时刻 $t_k$ 状态 |
| $z_k \in \text{R}^m$ | 测量 |
| $f(\cdot),\, h(\cdot)$ | 动力学、观测模型 |
| $P_{ww},\, P_{vv}$ | 过程噪声、测量噪声协方差 |
| $L_x$ | GMM 分量数 |
| $M$ | 伪时间步数（校正迭代次数） |
| $s_m \in [0,1]$ | 伪时间点，$0=s_1 \lt s_2 \lt \cdots \lt s_{M+1}=1$ |
...
先验参数上标写 $(\ell)\text{-}$，后验写 $(\ell)\text{+}$（如 $w_x^{(\ell)\text{-}}$），与论文先验/后验记号含义相同。
...
2. **GMM 过自信**：单步 GMF 中，许多分量一次吸收全部测量，尾部/低权重分量被大幅拉动，**NEES 显著偏大**（论文 2D 算例中 EGMF 中位 NEES $\gt 2$，极端可达 $\sim 7$）。
...
**式 (5)** — 似然乘积分解。设 $0=s_1 \lt s_2 \lt \cdots \lt s_{M+1}=1$，且 $\sum_m \Delta s_m = 1$，其中 $\Delta s_m = s_{m+1}-s_m$：
...
- $\Delta s_m \gt 0$ 时 $P_{vv}/\Delta s_m$ 非奇异（$P_{vv}$ 非奇异前提下）。
...
| 名称 | 式 (9) 近似 | 说明 |
|------|-------------|------|
| **DPF-EGMF** | EGMF：Taylor 线性化 $h(m_x)$、$H_x P H_x^{T}$ 等 | 一阶 Taylor；$H_x = \partial h / \partial x$ |
...
- 若 $s_m^{(\ell)} \gt 1-\bar{s}$，则 $s_m^{(\ell)} \leftarrow 1-\bar{s}$；
- 最大步数 $M_{\max}$；
- 最小步长 $\varepsilon$：若 $s \lt \varepsilon$，则 $s \leftarrow \varepsilon$；
- 若 $m=M_{\max}$ 且 $\bar{s}\lt 1$，最后一步取满剩余伪时间。
...
|--------|------|----------------|
| EKF/UKF | 差 | 不一致 |
| EGMF/UGMF | 改善 | 仍过自信（中位 NEES $\gt 2$） |
...
|--------|-------------|------|------|
| EGMF | 有偏；$\pm 3\sigma$ 不覆盖误差云 | 中位 NEES $\gt 1$，随周期恶化 | 单步失败 |
