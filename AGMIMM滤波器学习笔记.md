...
>
> **阅读说明**：display 公式单行；条件下标用 `\mid`；**符号表**与**表格**不用行内 `$`；**正文段落**中的公式已用 `$...$` 包裹。详见附录 B。
...
| 符号 | 含义 |
|------|------|
| x_k | 时刻 t_k 运动学状态（位置+速度，6 维） |
| x̃_k = [x_k^T, τ_k]^T | 联合状态（运动学 + 模式） |
| τ_k ∈ {1,…,M} | 目标模式；本文 M=2（弹道/机动） |
| π_ij | 模式转移概率 Pr(τ_k=j \| τ_{k-1}=i) |
| μ_k^{(j)} | 时刻 t_k 模式 j 的后验概率 |
| μ̄_{k\|k-1}^{(j)} | 时刻 t_k 模式 j 的先验（混合后）概率 |
| ω^{(ℓ,j)}, m^{(ℓ,j)}, P^{(ℓ,j)} | 模式 j 下第 ℓ 个 GM 分量权重、均值、协方差 |
| L^{(j)} | 模式 j 的 GM 分量数；合并目标数 L < L_max |
| f(·) | CR3BP 解流（离散动力学） |
| h^{(o)}(·) | 观测星 o 的角度测量模型 |
| Z_k^{(o)} | 观测星 o 的 Bernoulli RFS 测量（空集或单点） |
| p_D^{(o)}(x; S^{(o)}) | 状态相关的检测概率 |
| C(r,v) | Jacobi 常数（弹道积分量） |
| [LU], [TU] | CR3BP 归一化长度/时间单位 |

**上标约定**：混合后、预测前参数常记 **(·)_{k-1\|k-1}**；预测后验记 **(·)_{k\|k-1}**；测量更新后记 **(·)_k** 或 **(·)^{+}**（display 公式中用 `^{\text{+}}`）。
...
| 条件下标 | display 中用 `{k \mid k-1}`；表格/正文用 k\|k-1 纯文本 |
...
| 逆矩阵 | 用 `{...}^{-1}`，少用 `(...)^{-1}` |
| 表格 | 符号表 §0.3 用纯文本；其余表格单元格不写 `$...$` |
| 段落正文 | 可用行内 `$...$`；条件下标写 `{k \mid k-1}`，不用 `{k\|k-1}` |
