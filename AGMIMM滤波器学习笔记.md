...
**非线性 JMS 贝叶斯滤波目标**：递归计算联合状态的后验 pdf $p_{k|k}(\tilde{x}_k \mid Z_{0:k})$，其中 $Z_{0:k}$ 为直到 $t_k$ 的全部多传感器量测。
...
- 离散模式 $\tau_k \in \mathcal{M}$，转移概率 $\pi_{ij}$ 已知且时不变。
...
**【docx 文字说明 — 式 (19)】** 标准 Bayes 公式的联合版本：先验 $p_{k|k-1}(\tilde{x})$ 与当前量测似然 $g_k(Z_k \mid \tilde{x})$ 相乘后归一化。分母对 **所有连续状态** 与 **所有模式** 积分/求和。为显式区分模式，记 $\tilde{x}^{(j)} = [x_k^{\top}, \tau_k=j]^{\top}$。
...
1. **状态转移密度** $p_{k|k-1}(\tilde{x}_k^{(j)} \mid \tilde{x}_{k-1}^{(i)})$：由模式转移 $\pi_{ij}$、动力学 $f$、过程噪声（高斯）共同决定。
...
权重 $\pi_{ij}\mu_{k-1}^{(i)}/\bar{\mu}_{k|k-1}^{(j)}$ 是一种 **贝叶斯回溯**：给定当前模式 $j$，上一时刻模式为 $i$ 的后验概率。这正是 IMM **混合（mixing）** 步骤的精确表达。
...
| **AGMIMM** | **GMRC 聚类** 最优逼近 | $L \lt L_{\max}$ |
...
**【docx 文字说明 — 式 (62)–(64)】** 单传感器更新同时改变 GM 与 **模式概率** $\bar{\mu}^{(j)}$：分子含边缘似然预测积分（与式 50 同构），分母为所有模式分子之和。模式概率反映各模式对当前量测/未检测的 **边缘似然**——某模式分量在 $p_D$ 高区未检测，其边缘似然降、模式概率降，滤波器可 **自动识别** 当前运动模式。多传感器：$p_k^{(j)} = \Psi_k^{(O,j)} \circ \cdots \circ \Psi_k^{(1,j)}\, p_{k|k-1}^{(j)}$，$\mu_k^{(j)}$ 同样序贯更新。最终 $p_k(x) = \sum_j \mu_k^{(j)} p_k^{(j)}(x)$。
...
**式 (67)–(72)** — 预生成 **一维标准高斯** 最优分裂库，经缩放、平移、白化推广到多元沿 $\mathbf{u}$ 分裂。
...
| 判据 | 章节 | 目的 |
|------|------|------|
| 统计线性化误差 $c_{\text{SL}}$ | 5.1 / Sec. 4.1 | 线性化 $g(x)$ 误差超阈则分裂 |
| Jacobi 常数 $c_{\text{JC}}$ | 5.2 / Sec. 4.2 | $C$ 方差过大则分裂（廉价筛选） |
...
1. **预测后**：对 $p_{k|k-1}^{(j)}$ 应用 $\tilde{G}_c$（$c_{\text{SL}}(f)$, $c_{\text{JC}}$ 等）。
