...
>
> **阅读说明**：LaTeX 已针对 **GitHub 数学渲染**优化：display 公式单行；条件下标用 `\mid`；符号表与表格不用行内 `$`；`\begin{cases}` 已拆分为 (48a)/(48b) 等。详见附录 B。
...
其中 û^{(o)} = (r − r_obs^{(o)}) / ‖r − r_obs^{(o)}‖（见 display 公式）。
...
**非线性 JMS 贝叶斯滤波目标**：递归计算联合状态的后验 pdf p_{k|k}(x̃_k | Z_{0:k})，其中 Z_{0:k} 为直到 t_k 的全部多传感器量测。
...
- 连续状态 x_k ∈ R^6（位置+速度）。
- 离散模式 τ_k ∈ {1,…,M}，转移概率 π_ij 已知且时不变。
- 多传感器量测 Z_k = (Z_k^{(1)}, …, Z_k^{(O)})；每个 Z_k^{(o)} 为 Bernoulli RFS（空集或未检测到 / 单点量测）。
- **联合量测似然**（条件独立）：g_k(Z_k | x̃) = ∏_o g_k^{(o)}(Z_k^{(o)} | x̃)。
...
**【docx 文字说明 — 式 (19)】** 标准 Bayes 公式的联合版本：先验 p_{k|k-1}(x̃) 与当前量测似然 g_k(Z_k | x̃) 相乘后归一化。分母对 **所有连续状态** 与 **所有模式** 积分/求和。为显式区分模式，记 x̃^{(j)} = [x_k^T, τ_k=j]^T。

**【docx 文字说明 — 式 (20)】** 给定目标状态与模式，各传感器观测 **条件独立**（仅通过 x_k 耦合）。乘积结构使多传感器更新可 **顺序执行** 单传感器更新，极大简化计算。

**【docx 文字说明 — 式 (21)–(22)】** 量测条件独立时，可按任意顺序依次用第 o 个传感器的似然更新当前密度：Ψ_k^{(o)} 将已融合前 o−1 个传感器信息的密度，与第 o 个传感器似然相乘并归一化。最终等价于同时使用全部传感器。
...
1. **状态转移密度** p_{k|k-1}(x̃_k^{(j)} | x̃_{k-1}^{(i)})：由模式转移 π_ij、动力学 f、过程噪声（高斯）共同决定。
2. **上时刻后验** p_{k-1}(x̃_{k-1}^{(i)} | Z_{0:k-1})。
...
**【docx 文字说明 — 式 (33)–(34)】** 将预测步 (23) 写成模式条件形式：式 (32) 中 p_{k-1}(x_{k-1} | τ_k=j, Z_{0:k-1}) 即 **混合后验**。要预测当前模式 j 下的状态，并不知道 t_{k-1} 目标处于哪个模式，因此把上时刻各模式后验 p_{k-1}^{(i)}(x_{k-1}) 按 π_ij 与 μ_{k-1}^{(i)} 加权混合并归一化——这就是当前模式 j 下动力学传播的 **初始分布**。

权重 π_ij μ_{k-1}^{(i)} / μ̄_{k|k-1}^{(j)} 是一种 **贝叶斯回溯**：给定当前模式 j，上一时刻模式为 i 的后验概率。这正是 IMM **混合（mixing）** 步骤的精确表达。
...
**Step 4 — 归一化**：分母 μ̄_{k|k-1}^{(j)} = ∑_i π_ij μ_{k-1}^{(i)} 保证混合 pdf 积分为 1。
...
式 (34) 的混合后验 p_{k-1}(x_{k-1} | τ_k=j, Z_{0:k-1}) 是预测步的关键中间量：把上时刻各模式后验按 π_ij 加权，作为当前模式 j 下预测的起始分布。
...
**式 (49)** — 更新后 pdf 结构（未归一化；I_∅ 为未检测指示，I_+ 为其补）：

$$
[\Psi_k^{(o,j)} p^{(j)}](x) \propto I_{\emptyset} \sum_{\ell} (1-p_D) \omega^{(\ell,j)} N(x; m^{(\ell,j)}, P^{(\ell,j)}) + I_{+} \sum_{\ell} p_D \omega^{(\ell,j)} g(z \mid x) N(x; m^{(\ell,j)}, P^{(\ell,j)})
$$
...
**【docx 文字说明 — 式 (62)–(64)】** 单传感器更新同时改变 GM 与 **模式概率** μ̄^{(j)}：分子含边缘似然预测积分（与式 50 同构），分母为所有模式分子之和。模式概率反映各模式对当前量测/未检测的 **边缘似然**——某模式分量在 p_D 高区未检测，其边缘似然降、模式概率降，滤波器可 **自动识别** 当前运动模式。多传感器序贯更新 p_k^{(j)} 与 μ_k^{(j)}；最终 p_k(x) = ∑_j μ_k^{(j)} p_k^{(j)}(x)。
...
1. **预测后**：对 p_{k|k-1}^{(j)} 应用 G̃_c（c_SL(f), c_JC 等）。
